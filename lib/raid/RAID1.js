...
module.exports = class RAID1 extends aRAID {
    ...
    registerEvents() {
        ...
        this.watcher.on('add', (path, file) => add(EventFactory.getEvent('addFile', 'AF', path, file)));
        this.watcher.on('unlink', (path, file) => add(EventFactory.getEvent('removeFile', 'RF', path, file)));
        this.watcher.on('change', (path, file) => add(EventFactory.getEvent('changeFile', 'CF', path, file)));
